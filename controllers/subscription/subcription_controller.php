 ...
 if(isset($_POST['mail-subcription']))
{
  ...
  //    $token = mysqli_real_escape_string($dbc,strip_tags($_POST['token']));
    ...
    $sql_insert= mysqli_query($dbc,"INSERT INTO subscriptions
          (email)
            VALUES ('".$email."')") or die (mysqli_error($dbc));


    //log the action
    $action_reference = "Subscribed to the News Letter" . $fName;
