...
foreach ($_POST['sendToMany'] as $selectedEmail)
{
  ...
  else if(isset($_POST['post-emails1']))
  {
  ...
  mysqli_autocommit($dbc, FALSE);
         ...
         $sql_insert= mysqli_query($dbc,"INSERT INTO bulk_email
             (sendTo, message, recorded_by)
               VALUES ('".$sendTo."','".$long_desc2."', '".$recorded_by."')")
                or die (mysqli_error($dbc));
  ...
  foreach ($_POST['sendToMany'] as $selectedEmail)
  {
    ...
    $insert_approver = mysqli_query($dbc,"INSERT INTO bulk_email_multiple (mail_address,mail_details,  recorded_by)
    VALUES ('".$selectedEmail."','".$long_desc2."','".$recorded_by."')") or die (mysqli_error($dbc));
  ...
  //save emails Sent
  else if(isset($_POST['post-emails2']))
  {
  ...
  mysqli_autocommit($dbc, FALSE);
         ...
         $sql_insert= mysqli_query($dbc,"INSERT INTO bulk_email
             (sendTo, message, recorded_by)
               VALUES ('".$sendTo."','".$long_desc3."', '".$recorded_by."')")
                or die (mysqli_error($dbc));
  ...
  foreach ($_POST['sendToMany'] as $selectedEmail)
  {
    ...
    $insert_approver = mysqli_query($dbc,"INSERT INTO bulk_email_multiple (mail_address,mail_details,  recorded_by)
    VALUES ('".$selectedEmail."','".$long_desc3."','".$recorded_by."')") or die (mysqli_error($dbc));
    ...
    //save emails Sent
    else if(isset($_POST['post-emails3']))
    {
...
exit("failed");
  ...
  exit("failed");
    ...
    else if(isset($_POST['add_resource_history']))
    {
    ...
    $sql_insert= mysqli_query($dbc,"INSERT INTO resource_details
          (resource_id, resource, recorded_by)
            VALUES ('".$id."','".$long_desc."','".$recorded_by."')") or die (mysqli_error($dbc));

    //log the action
    $action_reference = "Addedd Resources with details" . $test_name;
    $action_name = "JTest APPLICATION request";
    $action_icon = "far fa-project-diagram text-success";
    $page_id = "About Me personal Details";
    $time_recorded = date('Y/m/d H:i:s');
    ...
    exit("failed");
      ...
      else if(isset($_POST['add_header_history']))
      {
      ...
      $sql_insert= mysqli_query($dbc,"INSERT INTO resources_header
            (resource_name, recorded_by)
              VALUES ('".$resource."','".$recorded_by."')") or die (mysqli_error($dbc));
